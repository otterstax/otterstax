            ...
            for (size_t i = 0; i < batch_nodes.size(); i++) {
                ...
                if (!connector_manager_->hasConnection(uid)) {
                    ...
                    co_return core::error_t(core::error_code_t::other_error,
                                            std::pmr::string{("no ClickHouse connection registered for '" + uid + "'").c_str(),
                                                             resource()});
                }
                ...
                if (node->type() == logical_plan::node_type::unused) {
                    ...
                    if (sn.has_raw_sql()) {
                        generated_queries.emplace_back(
                            sql_gen::replace_qualifiers(sn.raw_sql(), sn.qualifiers(), backend_type_t::ClickHouse));
                    } else {
                        generated_queries.emplace_back(
                ...
                if (node->type() == logical_plan::node_type::unused) {
                    ...
                    if (sn.has_raw_sql() && !sn.qualifiers().empty()) {
                        table_name = sn.qualifiers().front().name.collection;
                    }
                }
                auto named_overrides = connector_manager_->named_types_for(uid, table_name);
                auto data_converter =
                    [this, named_overrides = std::move(named_overrides)](
                        const std::vector<clickhouse::Block>& blocks) -> std::unique_ptr<data_chunk_t> {
                    return std::make_unique<data_chunk_t>(tsl::ch_to_chunk(this->resource(), blocks, named_overrides));
                };
