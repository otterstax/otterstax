...
// / service_misc.cpp by Wave 3. Registration here forwards each incoming RPC to
// the matching member coroutine through a thin lambda, so service.cpp is frozen
// the moment the interface lands.
...
void SparkConnectServiceImpl::start() {
    ...
    // shutdown-only. Each lambda forwards into the frozen member coroutine.
