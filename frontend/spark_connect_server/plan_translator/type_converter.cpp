...
namespace frontend::spark {
        ...
        switch (type.type()) {
            ...
            case logical_type::STRUCT: {
                ...
                for (size_t i = 0; i < children.size(); ++i) {
                    ...
                    // An unaliased field (an unnamed expression) is named by its
                    // position, as the Arrow encoder names it (result_encoder.cpp).
                    field->set_name(child.has_alias() ? child.alias() : "col" + std::to_string(i));
    ...
    ::spark::connect::DataType to_spark_schema(const complex_logical_type& schema) {
        OTX_ZONE_N("spark::to_spark_schema");
