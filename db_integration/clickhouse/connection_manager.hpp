...
namespace db_conn {
    ...
    class ChConnectionManager final : public actor_zeta::actor::actor_mixin<ChConnectionManager> {
    public:
        using is_cooperative_actor_type = void; // Required by actor_zeta::send() concept
        template<typename T>
        using unique_future = actor_zeta::unique_future<T>;

        ChConnectionManager(std::pmr::memory_resource* res, std::shared_ptr<chc::ConnectorManager> connector_manager);
        ...
        using dispatch_traits = actor_zeta::dispatch_traits<&ChConnectionManager::execute>;
