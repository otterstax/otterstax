...
namespace otterstax {
    ...
    public:
        ...
        result_t(T&& val) requires(trivial_store && !std::is_same_v<std::decay_t<T>, Error>)
            : value_(std::move(val))
            , error_(Error::no_error()) {}

        result_t(const T& val) requires(trivial_store&& std::is_copy_constructible_v<T> &&
                                        !std::is_same_v<std::decay_t<T>, Error>)
            : value_(val)
            , error_(Error::no_error()) {}

        // --- Success: implicit from unique_ptr<T> (non-trivial store) ---
        result_t(std::unique_ptr<T>&& ptr) requires(!trivial_store)
            : value_(std::move(ptr))
            , error_(Error::no_error()) {}
