    ...
    for (size_t i = 0; i < N; ++i) {
        boost::asio::co_spawn(
            ...
            [&, i]() -> boost::asio::awaitable<void> {
                results.push_back(
                    co_await otterstax::await_az_future(std::move(futs[i]), WAIT_TIMEOUT));
            },
            boost::asio::detached);
    }
    local.run();
...
// (integration/sql/connection_manager.cpp:32-49) plus the synchronous
// QueryHandleWaiter::wait (utility/wait_barrier.hpp:14-21) still serialise the
    ...
    auto baseline_start = std::chrono::steady_clock::now();
    {
        ...
        boost::asio::co_spawn(
            ...
            [&]() -> boost::asio::awaitable<void> {
                r = co_await otterstax::await_az_future(std::move(fut), WAIT_TIMEOUT);
            },
            boost::asio::detached);
        local.run();
