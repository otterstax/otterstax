...
//
// Spark Connect frontend — frozen interface for the gRPC service implementation.
//
// SparkConnectServiceImpl hosts an asio-grpc server (one GrpcContext event loop
// per hardware thread). The RPC handler coroutines are declared here as a frozen
// surface; their bodies live in service_execute_plan.cpp / service_analyze_plan.cpp
// / service_misc.cpp and are filled in by Wave 3. service.cpp only wires up the
// server (constructor, start()/stop(), RPC registration).
...
private:
    ...
    // -----------------------------------------------------------------------
    // RPC handler coroutines — frozen interface.
    //
    ...
    // implemented in separate translation units (Wave 3):
    ...
    // Defined alongside the handlers in Wave 3.
