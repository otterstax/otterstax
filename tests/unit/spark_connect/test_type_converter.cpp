...
TEST_CASE("type_converter: schema from flat column vector") {
    ...
    auto schema = frontend::spark::to_spark_schema(ct::complex_logical_type::create_struct("row", columns));
...
TEST_CASE("type_converter: schema from flat column vector falls back to colN for unaliased columns") {
    ...
    std::pmr::vector<ct::complex_logical_type> columns{resource};
    // An unaliased leaf type (an unnamed expression): its name is read only
    // behind has_alias(), so to_spark_schema must fall back to a stable colN.
