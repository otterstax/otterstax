...
if [ "$status" -eq 2 ]; then
    ...
    echo "    replay — recreate the process/container fresh). The server registers" >&2
    echo "    the connections declared in its config.yaml at startup; nothing to" >&2
    echo "    re-register by hand." >&2
fi
exit "$status"