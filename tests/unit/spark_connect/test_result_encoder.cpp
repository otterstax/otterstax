...
TEST_CASE("result_encoder: simple chunk encodes row count correctly") {
    ...
    chunk.set_value(0, 0, ct::logical_value_t{resource, int32_t{10}});
    chunk.set_value(0, 1, ct::logical_value_t{resource, int32_t{20}});
    chunk.set_value(0, 2, ct::logical_value_t{resource, int32_t{30}});
...
TEST_CASE("result_encoder: a column type the Arrow batch cannot carry is a conversion_failure") {
    std::pmr::synchronized_pool_resource pool;
    auto* resource = &pool;

    // The otterbrix C ABI converters throw on a type they have no Arrow mapping
    // for; the encoder must refuse the column with an error instead, since
    // nothing may throw out of it.
    std::pmr::vector<ct::complex_logical_type> types{resource};
    auto uuid_col = ct::complex_logical_type{ct::logical_type::UUID};
    uuid_col.set_alias("token");
    types.push_back(uuid_col);
    cv::data_chunk_t chunk(resource, types);
