...
PgConnectionManager::execute(session_hash_t id, ParsedQueryDataPtr data) {
    assert(data);
    try {
        ...
        // Create a handler to convert PostgreSQL results to data_chunk_t
        auto data_converter = [this](PGresult* result) -> std::unique_ptr<data_chunk_t> {
            return std::make_unique<data_chunk_t>(tsl::pg_to_chunk(this->resource(), result));
        };
