...
Scheduler::execute(session_hash_t id, shared_session_payload sdata, std::string sql) {
    try {
        ...
        if (has_external_nodes && get_backend_type(id) == backend_type_t::Unknown) {
            ...
            auto [needs_sched_catalog, catalog_future] = actor_zeta::send(
                catalog_manager_, &mysqlc::CatalogManager::update_backend_type, id, std::move(parsed));
            ...
            update_metadata(id, std::move(parsed));
        }
        ...
        if (backend == backend_type_t::MySQL || backend == backend_type_t::Mixed) {
            ...
            auto [needs_sched_sql, sql_future] = actor_zeta::send(
                sql_connection_manager_, &db_conn::SqlConnectionManager::execute, id, std::move(data_ptr));
            ...
            if (backend == backend_type_t::Mixed) {
                ...
                auto [needs_sched_pg, pg_future] = actor_zeta::send(
                    pg_connection_manager_, &db_conn::PgConnectionManager::execute, id, std::move(data_ptr));
        ...
        if (backend != backend_type_t::Otterbrix) {
            ...
            auto [needs_sched_otterbrix, otterbrix_future] = actor_zeta::send(
                otterbrix_manager_, &db_conn::OtterbrixManager::execute, id,
                std::move(data_ptr->otterbrix_params));
            auto cursor = co_await std::move(otterbrix_future);

            if (!cursor->is_success()) {
                std::string error_msg =
                    "Otterbrix execution failed: " + cursor->get_error().what;
            ...
            if (!cursor->is_success()) {
                complete_session_on_error(id, "Otterbrix execution failed: " + cursor->get_error().what);
                co_return;
            }
...
actor_zeta::unique_future<void>
Scheduler::execute_statement(session_hash_t id, shared_session_payload sdata) {
    try {
...
Scheduler::prepare_schema(session_hash_t id, shared_session_payload sdata, std::string sql) {
    try {
        ...
        if (parsed->otterbrix_params->external_nodes_count) {
            ...
            auto [needs_sched_catalog, catalog_future] = actor_zeta::send(
                catalog_manager_, &mysqlc::CatalogManager::get_catalog_schema, id, std::move(parsed));
            auto catalog_result = co_await std::move(catalog_future);
            ...
            auto [needs_sched_schema, schema_future] = actor_zeta::send(
                otterbrix_manager_, &db_conn::OtterbrixManager::get_schema,
                id, std::move(dependencies), std::move(data));
            ...
            finish_schema(id, std::move(cursor), std::move(parsed));
        }
    } catch (const std::exception& e) {
