...
namespace s3c {
    ...
    std::unique_ptr<s3c::IConnector> make_s3_connector(connect_params params, std::string alias) {
        return std::make_unique<s3c::S3Connector>(std::move(params), std::move(alias));
    }
    ...
    std::string ConnectorManager::addConnection(connect_params connection_param) {
        ...
        try {
            log_->debug("[S3ConnectorManager] Adding S3 connection: {} -> s3://{}/{}",
                        uuid, connection_param.bucket, connection_param.prefix);
            ...
            log_->error("[S3ConnectorManager] Error: {}", e.what());
            if (connections_.contains(uuid)) {
                connections_[uuid]->close();
                connections_.erase(uuid);
            }
            throw std::runtime_error("Add S3 connection error: " + std::string(e.what()));
        }
    }
    ...
    void ConnectorManager::removeConnection(const std::string& uuid) {
        ...
        if (conn == connections_.end()) {
            throw std::runtime_error("[S3ConnectorManager] Invalid connection uuid: " + uuid);
        }
        conn->second->close();
        connections_.erase(uuid);
        actor_zeta::send(catalog_manager_->address(),
